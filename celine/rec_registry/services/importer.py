...
async def replace_community_from_doc(session: AsyncSession, doc: CommunityImportDoc) -> dict[str, int]:
    ...
    existing = await session.scalar(select(Community).where(Community.key == mapped.community_key))

    if existing:
        # Delete dependents explicitly (avoid relying on ORM cascades for bulk ops)
        cid = existing.id
        await session.execute(delete(TopologyEdge).where(TopologyEdge.community_id == cid))
        ...
        await session.execute(delete(Participant).where(Participant.community_id == cid))
    ...
    for p in mapped.participants:
        p.community_id = mapped.community.id
        session.add(p)
    await session.flush()

    for s in mapped.sites:
        s.community_id = mapped.community.id
        session.add(s)
    await session.flush()

    for m in mapped.meters:
        m.community_id = mapped.community.id
        session.add(m)
    await session.flush()

    for a in mapped.assets:
        a.community_id = mapped.community.id
        session.add(a)
    await session.flush()

    for t in mapped.tariffs:
        t.community_id = mapped.community.id
        session.add(t)
    await session.flush()

    for ts in mapped.timeseries:
        ts.community_id = mapped.community.id
        session.add(ts)
    await session.flush()

    for mem in mapped.memberships:
        mem.community_id = mapped.community.id
        session.add(mem)
    await session.flush()

    for e in mapped.topology_edges:
        e.community_id = mapped.community.id
        session.add(e)
    await session.flush()
