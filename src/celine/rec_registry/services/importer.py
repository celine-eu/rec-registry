...
async def replacement_import_bundle(
    ...
    *,
    ...
    # Count assets (from members + community)
    for member_key, member in bundle.members.items():
        if member.assets:
            inserted["asset"] += _count_assets(member.assets)
    
    if bundle.community.assets:
        inserted["asset"] += _count_assets(bundle.community.assets)
    ...
    community = Community(
        ...
        areas=areas_dict,
        extra=_extract_extra(bundle.community, {"id", "name", "description", "areas", "assets"}),
    )
    ...
    for member_key, member_data in bundle.members.items():
        member = Member(
            ...
            extra=_extract_extra(member_data, {"user_id", "name", "role", "area", "status", "assets"}),
        )
        session.add(member)
    ...
    # Create community-owned assets (if any)
    # Note: Community-owned assets need a placeholder member or different handling
    # For now, we'll skip community assets if no members exist
    if bundle.community.assets and _count_assets(bundle.community.assets) > 0:
        warnings.append("Community-owned assets are not yet supported; skipped")
...
async def _create_assets(
    ...
    *,
    ...
    base_exclude = {"name", "relationships"}
